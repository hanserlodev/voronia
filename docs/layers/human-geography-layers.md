...
| Borders | ✅ | **Cadenas continuas FMG** (`draw-borders.ts` portado): dos pasadas (provincias intra-estado → estados) con dedupe orientado `id>neibId` + retry por celda, walk `get_vertices_line` con gates de tierra (nunca cruzan agua, islas = cadenas separadas), loops repiten vértice inicial sin Z. Dash/dots fluyen **a lo largo de toda la cadena** (fase continua como SVG). Estilos exactos: state `#56566d` w1 dash2 · province `#56566d` w0.5 dots "0 2" round. Culture borders = extensión Voronia documentada (FMG no las tiene), mismo walker |
