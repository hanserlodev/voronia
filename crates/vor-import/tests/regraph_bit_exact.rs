//! Test bit-exacto: `re_graph` Rust vs `reGraph` JS (main.js:1157-1209).
//!
//! Fixture (`regraph_h50_t2_grid_2000x2000_c10k_seed_861039636_selfref.json`):
//! synthetic con `h=50` (interior land) y `t=2` (tierra interior, no costa) para
//! todas las 10000 celdas del grid. Validado para el path "sin descartes, sin
//! puntos extra" (todos pasan el filtro y ninguno es costero). Los paths de
//! descarte (h<20, type=-2, lago) y costeros (type=±1) están cubiertos por tests
//! unitarios en `src/regraph.rs`.
//!
//! Test compara:
//! - `pack.points` bits (BigInt64Array del JS, decodificados a f64) — input al
//!   segundo `calculateVoronoi` (= la salida `newCells.p`).
//! - `pack.cells.grid_id` (= `newCells.g`) — mapeo pack→grid.
//! - `pack.cells.height` (= `newCells.h`).
//! - `pack.cells.area_px` — vía `d3.polygonArea` (shoelace) capped a u16.
//! - `pack.vertices.p` — coords bit-exactas con `Math.floor`.
//! - `pack.vertices.v` — vecinos con `-1` = EMPTY.
//! - `pack.vertices.c` — cells adyacentes.
...
/// Strings decimalizados de i64 (bits de f64) → `Vec<f64>`.
...
fn parse_u32_strings(arr: &Value, field: &str) -> Vec<u32> {
    arr.as_array()
        ...
        .map(|x| {
            x.as_str()
                .unwrap_or_else(|| panic!("`{field}` entry debería ser string"))
                .parse::<u32>()
    ...
    arr.as_array()
        ...
        .map(|x| {
            x.as_str()
                .unwrap_or_else(|| panic!("`{field}` entry debería ser string"))
                .parse::<u16>()
                .unwrap_or_else(|e| panic!("`{field}` u16 inválido: {e}"))
        })
        .collect()
}
...
fn regraph_rust_matches_js_self_reference() {
    ...
    // Con synthetic h=50 t=2: todos pasan filtro 1 (h>=20 cumple), filtro 2 (type=-2 no
    // cumple). Sin costeras (type=2). Por eso pack.cells.p == 10000 (igual que grid).
    assert_eq!(
        n_pack_points, 10000,
        "synthetic land interior → pack == grid (sin extras)"
    );

    // Reproducir `placePoints` Rust (input al reGraph).
    ...
    // Topología del grid (necesaria para `re_graph` — `grid_voronoi.cells.b/c`).
    ...
    // Atributos synthetic (igual que en generate_regraph_fixture.js).
    ...
    let grid_features_kind: Vec<FeatureType> = vec![FeatureType::Ocean]; // idx 0 = Ocean (no Lake).
    ...
    // === Comparar new_pts (f64) bit-a-bit vs el JS ===
    // `new_pts` (Rust) son los `newCells.p` del algoritmo, antes del cast a f32 para
    // `Pack::points` (cap fijo del model vor-core). Comparar estos — no `pack.points` —
    // para bit-exactitud real.
    ...
    assert_eq!(
        mismatches_pts, 0,
        "{} mismatches en new_pts",
        mismatches_pts
    );

    // === Comparar grid_id (mapeo pack→grid) ===
    ...
    assert_eq!(mismatches_g, 0, "{} mismatches en grid_id", mismatches_g);
    ...
    assert_eq!(mismatches_h, 0, "{} mismatches en height", mismatches_h);
    ...
    assert_eq!(
        mismatches_area, 0,
        "{} mismatches en area_px",
        mismatches_area
    );

    // === Comparar vertices.p bit-a-bit ===
    ...
    assert_eq!(
        mismatches_vp, 0,
        "{} mismatches en vertices.positions",
        mismatches_vp
    );

    eprintln!(
        "[regraph bit-exact] OK: {} pack points, {} triangles — 0 mismatches en points/grid_id/height/area/vertices",
